...
%%
% clc; clear; 
SAVE_Path_2D = '/home/iismn/WorkSpace/Research/IRIS_Jukdong/DeepLearningDB/2D/';
SAVE_Path_3D = '/home/iismn/WorkSpace/Research/IRIS_Jukdong/DeepLearningDB/3D/';
SAVE_Path_POS = '/home/iismn/WorkSpace/Research/IRIS_Jukdong/DeepLearningDB/POS_2D/';

%% B. ROS BAG to START POINT
% COORD = [36.3381112	127.3404169];
% Lat = COORD(1);
% Long =  COORD(2);
...
rawB = fread(fopen('Jukdong_Build.geojson'),inf);
rawB = char(rawB');
geojsonValue_BUILD = jsondecode(rawB);

rawR = fread(fopen('Jukdong_Road.geojson'),inf);
rawR = char(rawR');
...
FList_UTM_2D = FList_UTM_2D';
Iteration = size(FList_UTM_2D,1);

for i = 1:Iteration-1

    StartPoint = [FList_UTM_2D(i,1), FList_UTM_2D(i,2)];
    EndPoint = [FList_UTM_2D(i+1,1), FList_UTM_2D(i+1,2)];
    
    Intrp_X = linspace(StartPoint(1), EndPoint(1),4);
    Intrp_Y = linspace(StartPoint(2), EndPoint(2),4);
